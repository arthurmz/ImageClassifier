 ...
 * Exporta uma nova versăo da imagem para verificar visualmente o processo de contagem.
 * @author Arthur.
 *
 */
public class Main {
		...
		try {
		    ...
		    for(int i = 0; i < w; i++) {
		    	for (int j = 0; j < h; j++) {
		    		...
		    		//FILTRANDO A VARIAÇĂO DE COR E CONTANDO A QUANTIDADE DE CADA PIXEL.
		    		if (isBranco(rgb)) {
		    			pixelsBrancos++;
		    		}
		    		...
		    		//DECIDINDO A COR DOS PIXELS RESTANTES (BORDAS PRETAS E CORES COM POUCA SATURAÇĂO)
		    		else {
		    ...
		    System.out.println("Pixels năo contabilizados: " + naoContabilizados + " (" + areaNaoContabilizada + "%)");
